...
for path in [
    ...
    "docs/BUILDING.md", "docs/PLUGIN_API.md", "docs/PLUGIN_FORMAT.md", "docs/FDROID.md",
    "docs/DEVELOPER_GUIDE.md", "docs/SAVE_SYSTEM.md", "docs/SETTINGS_SYSTEM.md",
    "docs/MIGRATION_COMPATIBILITY.md", "docs/TRUSTED_NATIVE.md", "VALIDATION_REPORT.md", "licenses/YOUNGLION_MOD_LICENSE.txt",
...
require("CMakeLists.txt", "LEOMINIGAMES_FDROID")
...
require("src/core/ModManager.cpp", "trustedLevel3")
...
# App-side source headers use the selected SPDX identifier.
...
# Android release identity must remain stable.
manifest_text = (ROOT / "android/AndroidManifest.xml").read_text(encoding="utf-8")
for token in ('package="xyz.younglion.leominigames"', 'android:versionCode="620"', 'android:versionName="0.6.2"', 'android.permission.VIBRATE'):
    if token not in manifest_text:
        errors.append(f"Android manifest missing {token}")

if errors:
    print("LeoMiniGames v0.6.2 compatibility validation FAILED")
    for error in errors:
        print("ERROR:", error)
    sys.exit(1)

print("LeoMiniGames v0.6.2 compatibility validation OK")
